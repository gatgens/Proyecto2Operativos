...
/**
 * Funcion que busca el inodo mediante el path
 * @param filename
 * @return
 */
...
/**
 * Funcion para override my_rename, cambia el
 * nombre del path en el inode
 * @param path1
 * @param path2
 * @return
 */
...
/**
 * Funcion que busca el indice del inodo padre del nodo actual
 * @param path
 * @return
 */
...
/**
 * Obtiene el nombre del archivo o del directorio desde el path
 * @param path
 * @return
 */
...
/**
 * Actualiza la lista de inodos desde la lista de inodos del superbloque
 * @param nodo_libre
 */
...
/**
 * Crea un inodo a partir del path
 * @param path
 * @param size
 * @param modo
 * @param es_fichero
...
/**
 * Funcion para override mkdir, crea un inodo para el directorio
 * @param path
 * @param mode
 * @return
 */
...
/**
 * Funcion para override rmdir, borra el inodo
 * si tiene ficheros no se elimina
 * @param path
 * @return
 */
...
/**
 * funcion para override readdir, verifica
 * que el inodo exista y que sea el padre correcto
 * por si el directorio se llama igual a otro
 * @param path
 * @param buf
 * @param filler
 * @param offset
...
/**
 * funcion para override open, busca el inodo desde el path
 * @param path
 * @param fi
 * @return
 */
...
static int my_getattr(const char *path, struct stat *st){
    ...
    if(!strcmp(path,"/")){
        ...
        st->st_atime = lista_inode[nodo].fecha_acceso; //
        st->st_mtime = lista_inode[nodo].fecha_modificacion;

        if(lista_inode[nodo].es_fichero){
            st->st_mode = S_IFDIR | 0755;
            st->st_nlink = 2;
        }
        else{
          st->st_mode = S_IFREG | 0644;
          st->st_nlink = 1;
          st->st_size = lista_inode[nodo].cantidad_punteros;
        }
    }
    else{
        return -2;
...
/**
 * funcion para override statfs
 * copia la informacion del estado del fs
 * @param path
 * @param fs
 * @return
 */
...
/**
 * funcion para override write, busca si el archivo ya existe para
 * reemplazarlo, crea los qr a partir de la informacion a escribir
 * @param path
 * @param buffer
 * @param size
 * @param offset
...
/**
 * Struct para decirle a fuse las funciones
 */
...
/**
 * FUncion para inicializar los inodos de memoria
 */
...
int main(int argc, char *argv[]){
    ...
    // Inicializa la pila
    crearSuperBloque(100);

    // Lee archivos de datos persistentes (respaldo)
    FILE *estructura,*nodos;
